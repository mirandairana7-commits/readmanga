...
<head>
    ...
    <style>
        body { background-color: #121212; color: #e0e0e0; overscroll-behavior-y: none; margin: 0; padding: 0; }
        ...
        .mode-webtoon .reader-image { width: 100%; }
        #brightness-layer { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: black; opacity: 0; pointer-events: none; z-index: 9999; }
...
<body class="antialiased mode-webtoon" id="bodyReader">
    ...
    <main class="min-h-screen bg-[#121212]" id="readerArea">
        <div class="reader-container max-w-[800px]" id="imageContainer">
            ...
            if (mysqli_num_rows($images) > 0) {
                ...
                while ($img = mysqli_fetch_assoc($images)): 
                    $count++;
                    // LOGIKA PATH GAMBAR (PENTING!)
                    $imgSrc = $img['image_path'];
                    if (strpos($imgSrc, 'http') !== 0) {
                        // Jika bukan http, berarti file lokal -> tambah base_url
    ...
    <div class="fixed bottom-6 right-4 z-[60] flex flex-col gap-3 ui-controls" id="floatingControls">
        ...
        <button onclick="scrollToBottom()" class="bg-black/70 backdrop-blur text-white w-10 h-10 rounded-full border border-gray-600 flex items-center justify-center"><i class="fas fa-arrow-down text-sm"></i></button>
    ...
    <div id="settingsPanel" class="fixed bottom-0 left-0 w-full bg-gray-900/95 backdrop-blur-xl border-t border-gray-700 z-[70] transform translate-y-full transition-transform duration-300 rounded-t-2xl shadow-2xl">
        <div class="max-w-2xl mx-auto p-6 pb-8">
            ...
            <div class="mb-6">
                <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Tampilan</label>
            ...
            <div class="mb-4" id="gapControl">
                <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Jarak Gambar</label>
                <div class="flex gap-2">
                    <button onclick="setGap(0)" class="flex-1 py-2 bg-gray-800 border border-gray-700 rounded text-xs hover:bg-gray-700">Rapat</button>
                    <button onclick="setGap(10)" class="flex-1 py-2 bg-gray-800 border border-gray-700 rounded text-xs hover:bg-gray-700">Sedang</button>
                    <button onclick="setGap(20)" class="flex-1 py-2 bg-gray-800 border border-gray-700 rounded text-xs hover:bg-gray-700">Luas</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            setMode(readingMode);
            const savedGap = localStorage.getItem('readerGap');
            if(savedGap !== null) setGap(savedGap);
        ...
        function setMode(mode) {
            ...
            const gapCtrl = document.getElementById('gapControl');
            ...
            if (mode === 'webtoon') {
                ...
                bottomNav.classList.remove('hidden'); pageInd.classList.add('hidden'); gapCtrl.classList.remove('hidden');
            } else {
                ...
                bottomNav.classList.add('hidden'); pageInd.classList.remove('hidden'); gapCtrl.classList.add('hidden');
            }
        }
        function showPage(num) {
            ...
            if (num > totalPages) {
                if(nextUrl) {
                    if(confirm("Lanjut ke Chapter berikutnya?")) window.location.href = nextUrl;
                } else {
        ...
        function setBrightness(val) { document.getElementById('brightness-layer').style.opacity = (100 - val) / 100; document.getElementById('brightVal').innerText = val + '%'; }
        function setGap(px) { const images = document.querySelectorAll('.reader-image'); images.forEach(img => img.style.marginBottom = px + 'px'); localStorage.setItem('readerGap', px); }
        function scrollToTop() { window.scrollTo({ top: 0, behavior: 'smooth' }); }
        function scrollToBottom() { window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' }); }
        document.addEventListener('keydown', (e) => { if (readingMode === 'single') { if (e.key === 'ArrowRight' || e.key === ' ') nextPage(); if (e.key === 'ArrowLeft') prevPage(); } });
    </script>
</body>
</html>